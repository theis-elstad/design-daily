...
import { resolveFixPrompt } from '@/lib/jewelgen-prompts'
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const parts = [
      ...
      textPart(resolveFixPrompt(promptRow?.content ?? null)),
    ]

    const images = await callGemini(parts)
    const result = images[0]

    // Store the fixed image in Supabase Storage
    const buffer = Uint8Array.from(atob(result.base64), (c) => c.charCodeAt(0))
    const ext = result.mimeType.includes('png') ? 'png' : 'jpg'
    const filePath = `${user.id}/fixed-${Date.now()}.${ext}`
    ...
    if (uploadError) {
      return NextResponse.json(
        { error: `Failed to store fixed image: ${uploadError.message}` },
        { status: 500 }
      )
    }
