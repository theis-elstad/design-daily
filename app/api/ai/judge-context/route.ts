...
import { judgeContextPrompt } from '@/lib/ai-prompts'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const recentSubmissions = typedSubs.map((s) => ({
    ...
    const recentFeedback = typedSubs
      .flatMap((s) => s.ratings?.map((r) => r.comment).filter(Boolean) || [])
      .filter((c): c is string => c !== null)
      .slice(0, 5)

    const { system, user: userPrompt } = judgeContextPrompt(
      designerName,
      recentSubmissions,
      recentFeedback
    )
    ...
    const { data: summary, error } = await (supabase.from('ai_summaries') as any)
      .insert({
        ...
        content,
        metadata: { submission_count: recentSubmissions.length },
      })
